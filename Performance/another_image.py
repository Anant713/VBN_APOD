...
def make_image(xdeg,ydeg,zdeg,x,y,z) :
    # Define image size
    ...
    markers_3D = np.array([
        [10, 0, 0],    # Marker 1 (3m in front)
        [0, 10, 0],  # Marker 2 (right)
        [-10, 0, 0],  # Marker 3 (up)
        [0, -10, 0],# Marker 4 (diagonal)
        [0, 0, -10]  # A closer marker at Z=2m
        # [50, 0, 30],
        # [-50,0,30]
    ], dtype=np.float32)
    ...
    translation_vector = np.array([[x], [y], [z]], dtype=np.float32)  # Shift right & down
