...
markers_3D = np.array([
    [20, 0, 0],    # Marker 1 (3m in front)
    [0, 20, 0],  # Marker 2 (right)
    [-20, 0, 0],  # Marker 3 (up)
    [0, -20, 0],# Marker 4 (diagonal)
    [0, 0, -20]  # A closer marker at Z=2m
    # [50, 0, 30],
    # [-50,0,30]
], dtype=np.float32)
...
theta_x, theta_y, theta_z = np.radians([0, 10, 0])  # Rotation in X, Y, Z (in degrees)
